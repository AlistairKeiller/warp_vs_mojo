...
comptime input_dtype = DType.float16
...
def _load_frag[
    ...
    base: UnsafePointer[SIMD[dtype, 1], ...],
    stride: Int,
) -> SIMD[dtype, 8]:
    var tid = lane_id()
...
def _store_frag[
    ...
    base: UnsafePointer[mut=True, SIMD[dtype, 1], ...],
    stride: Int,
    frag: SIMD[dtype, 8],
):
    var tid = lane_id()
...
@always_inline
def _apple_mma(
    mut d: SIMD[accum_dtype, 8],
    a: SIMD[input_dtype, 8],
    b: SIMD[input_dtype, 8],
    c: SIMD[accum_dtype, 8],
):
    d = rebind[type_of(d)](
        llvm_intrinsic[
...
def bench[size: Int]() raises:
    ...
    def kernel(
        A: TileTensor[input_dtype, type_of(a_layout), MutAnyOrigin],
        B: TileTensor[input_dtype, type_of(b_layout), MutAnyOrigin],
        C: TileTensor[accum_dtype, type_of(c_layout), MutAnyOrigin],
    ):
        ...
        var a_smem = stack_allocation[input_dtype, address_space=AddressSpace.SHARED](
            a_smem_layout
        )
        var b_smem = stack_allocation[input_dtype, address_space=AddressSpace.SHARED](
            b_smem_layout
        )
        ...
        var accum = InlineArray[SIMD[accum_dtype, 8], N_ACC](
            uninitialized=True
        )
        comptime for i in range(N_ACC):
            accum[i] = SIMD[accum_dtype, 8](0)
        ...
        comptime for tm in range(WARP_TILES_M):
            comptime for tn in range(WARP_TILES_N):
                ...
                _store_frag[accum_dtype](c_base, N, accum[tm * WARP_TILES_N + tn])

    print("\n--- Size ", M, " (F16 input, F32 accum) ---")

    var ctx = DeviceContext()
    var buf_a = ctx.enqueue_create_buffer[input_dtype](M * K)
    var buf_b = ctx.enqueue_create_buffer[input_dtype](K * N)
    var buf_c = ctx.enqueue_create_buffer[accum_dtype](M * N)
    var ha = ctx.enqueue_create_host_buffer[input_dtype](M * K)
    var hb = ctx.enqueue_create_host_buffer[input_dtype](K * N)
    var hc = ctx.enqueue_create_host_buffer[accum_dtype](M * N)
    ctx.synchronize()
    ...
    for i in range(M):
        for j in range(K):
            ha_t[i, j] = SIMD[input_dtype](Float32(Int(i * K + j) % 100) / 100.0)
    for i in range(K):
        for j in range(N):
            hb_t[i, j] = SIMD[input_dtype](Float32(Int(i * N + j * 7) % 100) / 100.0)
    ...
    for i in range(M):
        for j in range(N):
            ...
            for k in range(K):
                expected += Float64(ha_t[i, k]) * Float64(hb_t[k, j])
            var err = abs(Float64(hc_t[i, j]) - expected)
            if err > max_err:
                max_err = err
    print("Max error: ", Float32(max_err))
...
def main() raises:
    comptime if not has_accelerator():
        ...
        print("--- Mojo GPU Matmul F16 (M5 Pro, simdgroup MMA) ---")
        bench[256]()
        bench[512]()
        bench[1024]()
        bench[2048]()
